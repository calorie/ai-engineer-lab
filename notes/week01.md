...
- 最初のデータセットとして Breast Cancer Wisconsin を選定した
- `experiments/001_baseline.ipynb` を作成した
- 初回ベースライン実験の目的、完了条件、実装セルの雛形を用意した

データセットを選んだ理由:

- scikit-learn に内蔵されており、外部データ取得が不要で再現しやすい
- 二値分類問題であり、accuracy, precision, recall, F1, confusion matrix の練習に向いている
- 特徴量が数値で揃っており、初回は前処理よりも評価設計とベースライン構築に集中できる

うまくいったこと:

- Week 2 のベースライン実験へ進むための題材を固定できた
- 実験Notebookの入口を作成できた
...
- `experiments/001_baseline.ipynb` を実行する
- 出力された評価指標をNotebookの「観察」に記録する
- confusion matrix から、どちらの誤分類が問題かを説明する
